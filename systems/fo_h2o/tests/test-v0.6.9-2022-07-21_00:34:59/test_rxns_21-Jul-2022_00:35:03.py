...
class Testfo_h2o_hydration:
    ...
    def test_Gamma_i(self):
        ...
        answer =[-1.6941351899607588, 1.668403275657492, 0.35112403310403056, -0.32539211880076385]
        assert(allclose_float_list(test,answer,rtol=1e-05,atol=1e-08))
    ...
    def test_Mu(self):
        ...
        answer =[[-2217084.332036724], [-4463217.9007785525], [-954530.8351882801], [-247654.02700368594]]
        assert(allclose_float_list(test,answer,rtol=1e-05,atol=1e-08))
    ...
    def test_beta(self):
        test = self.phase.beta(423.15, 10.0, [[1.0], [1.0], [1.0], [1.0]])
        answer =[7.88292739426994e-07, 1.8040178554526272e-06, 2.01452893385023e-06, 6.186511197374323e-05]
        assert(allclose_float_list(test,answer,rtol=1e-05,atol=1e-08))
    ...
    def test_dGamma_i_dPhi(self):
        ...
        answer =[[-0.9411862166448659, -0.0, -0.0, -8.470675949803795], [0.9268907086986066, 0.0, 0.0, 8.342016378287461], [0.19506890728001694, 0.0, 0.0, 1.7556201655201529], [-0.1807733993337577, -0.0, -0.0, -1.6269605940038194]]
        assert(allclose_float_list(test,answer,rtol=1e-05,atol=1e-08))

    def test_dGamma_i_dT(self):
        test = self.phase.dGamma_i_dT(423.15, 10.0, [[1.0], [1.0], [1.0], [1.0]], [0.9, 0.0, 0.0, 0.1])
        answer =[0.002213605174349331, -0.0021799831240046095, -0.00045878863807527145, 0.00042516658773055014]
        assert(allclose_float_list(test,answer,rtol=1e-05,atol=1e-08))
    ...
    def test_dGamma_ik_dPhi(self):
        ...
        answer =[[0.9268907086986066, 0.0, 0.0, 8.342016378287461]]
        assert(allclose_float_list(test,answer,rtol=1e-05,atol=1e-08))
    ...
    def test_s(self):
        ...
        answer =[0.9907161200838106, 1.1772799456824536, 1.5972083523946317, 5.356949951174805]
        assert(allclose_float_list(test,answer,rtol=1e-05,atol=1e-08))
