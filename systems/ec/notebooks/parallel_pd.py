...
x,y = np.indices(rho_final.shape)
rhonan = np.isnan(rho_final)
rho_final[np.isnan(rho_final)] = griddata((x[~rhonan], y[~rhonan]),rho_final[~rhonan],(x[rhonan], y[rhonan]))
...
T_zoomed = ndimage.zoom(T_final, 5)
P_zoomed = ndimage.zoom(P_final, 5)
rho_zoomed = ndimage.zoom(rho_final, 5)

s = axi.contourf(T_zoomed,P_zoomed,rho_zoomed,levels=density_levels,alpha=0.75,cmap=density_cmap)
axi.contour(T_zoomed,P_zoomed,rho_zoomed,levels=density_levels,alpha=1,cmap=density_cmap)
axi.contour(T_zoomed,P_zoomed,rho_zoomed,levels=highlight_densities,alpha=1,colors="black")
plt.xlim([Tmin, Tmax])
plt.ylim([Pmin,Pmax])
...
fig = plt.figure(figsize=(12,14))
axi = fig.add_subplot(1,1,1)
s = plt.scatter(T_final,P_final,c=stime_final,s=100,alpha=0.75,cmap=stime_cmap,norm=mpl.colors.LogNorm())
...
if (composition=="hacker_2015_md_xenolith"):
    ...
    s = axi.contourf(T_final, P_final, rho_final, levels=density_levels, alpha=0.75, cmap=density_cmap)
    axi.contour(T_final, P_final, rho_final, levels=density_levels, alpha=1, cmap=density_cmap)
    plt.xlim([Tmin, Tmax])
    ...
    s = axi.contourf(T_final, P_final, rho_eq, levels=density_levels, alpha=0.75, cmap=density_cmap)
    axi.contour(T_final, P_final, rho_eq, levels=density_levels, alpha=1, cmap=density_cmap)
    plt.xlim([Tmin, Tmax])
    plt.ylim([Pmin,Pmax])
    plt.colorbar(mappable=s, location="bottom",ticks=density_ticks, label="10$^2$ kg/m$^3$")
    ...
    diff = (rho_final-rho_eq)
    s=axi.imshow(diff,extent=(Tmin,Tmax,Pmin,Pmax),origin="lower",cmap=diff_cmap,aspect="auto",vmin=-2,vmax=2)
    axi.contour(T_final ,P_final, diff, levels=levels,**contour_kwargs)
...
axi = fig.add_subplot(1,3,2)
s = axi.contourf(T_final, P_final, rho_pyrolite, levels=density_levels, alpha=0.75, cmap=density_cmap)
axi.contour(T_final, P_final, rho_pyrolite, levels=density_levels, alpha=1, cmap=density_cmap)
plt.xlim([Tmin, Tmax])
plt.ylim([Pmin,Pmax])
plt.colorbar(mappable=s, location="bottom", ticks=density_ticks, label="10$^2$ kg/m$^3$")
...
axi = fig.add_subplot(1,3,3)
diff = (rho_final-rho_pyrolite)*100 # g/cm3
maxval = np.ceil(np.nanmax(np.absolute(diff))/100)*100+50
levels = np.arange(-maxval,maxval+50,50)
s=axi.imshow(diff,extent=(Tmin,Tmax,Pmin,Pmax),origin="lower",cmap=diff_cmap,aspect="auto",vmin=-maxval,vmax=maxval)
axi.contour(T_final ,P_final, diff, levels=levels,**contour_kwargs)
