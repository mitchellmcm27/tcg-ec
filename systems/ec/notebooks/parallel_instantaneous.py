...
def get_initial_state(rxn,scenario):
    ...
    P1 = crustal_rho * gravity * z1/1e5 # bar

    Cik0, mi0, rho0 = get_initial_composition(T0,P0,rxn,composition)

    return T0, P0, Cik0, mi0, rho0, qs0, T1, qs1, P1

def setup_ics(scenario):
    rxn = get_reaction(rxn_name)
    T0, P0, Cik0, mi0, rho0,qs0,T1,qs1,P1 = get_initial_state(rxn,scenario)
    scenario["T0"] = T0
    scenario["T1"] = T1
    scenario["qs0"] = qs0
    scenario["qs1"] = qs1
...
def run_experiment(scenario):
    ...
    ode = ScipyPDReactiveODE(rxn)
    print(T1,P1)
    ode.solve(T1, P1, mi0, Cik0, 1, Da=Da, eps=eps, rtol=rtol,atol=atol, t_eval=np.linspace(0,1,1000))
    y = ode.sol.y
    mi_times = y[:I].T
    Cik_times = y[I:I+K].T
...
for out in outs:
    ...
    if max_rho > max_rho_py:
        ...
        if len(noncritical_indices) == 0:
            ...
            critical_depth = time[critical_index]
    else:
        critical_time = 1. # approx. coesite transition?, can change this later
...
times = [o['critical_time'] for o in selected_outputs]
...
fig = plt.figure(figsize=(5,7))
s = plt.scatter([o["Da"] for o in selected_outputs], times, s=sizes, c=codes, cmap="tab10", alpha=0.5)
...
plt.semilogx()
plt.xlabel("Da")
plt.ylabel("Time to foundering (Myr)")
plt.savefig(Path(output_path,"{}.{}".format("_critical_time", "pdf")), metadata=pdf_metadata, bbox_extra_artists=(legend1,legend2), bbox_inches='tight')
plt.savefig(Path(output_path,"{}.{}".format("_critical_time", "png")),bbox_extra_artists=(legend1,legend2), bbox_inches='tight')
...
for composition in compositions:
    for tectonic_setting in tectonic_settings:
        ...
        for i, obj in enumerate(outs_c):
            ax = axes["rho"]
            ax.plot(obj["rho"], obj["time"])

        rho_pyrolite=ipyrolite((outs_c[0]["T1"], outs_c[0]["P1"]/1e4))
        rho_harzburgite=iharzburgite((outs_c[0]["T1"],outs_c[0]["P1"]/1e4))

        ax.plot([rho_pyrolite/10, rho_pyrolite/10],[0, 1], "r:")
        ax.plot([rho_harzburgite/10,rho_harzburgite/10], [0,1], "m:")
        ...
        ax = axes["T"]
        ax.plot(T-273.15, base["time"], linewidth=2)
        ax.set_xlabel("T (°C)")
        ax2 = ax.twinx()
        ...
        for j, obj in enumerate(outs_c):
            ...
            axes["An"].plot(XAn, obj["time"])
            axes["Jd"].plot(XJd, obj["time"])
