...
result = 1.0*x1*(26000.0*n1*n2 + 1.0*x0*(8.3144626181532395*T*(n1*log(n1*x1) + n2*log(n2*x1)) + n1*(*endmember[0].mu0)(T, P) + n2*(*endmember[1].mu0)(T, P)));
    return result;
}
...
static void coder_dgdn(double T, double P, double n[2], double result[2]) {
    ...
    double x8 = n2*x5;
    double x9 = log(x8);
    double x10 = 8.3144626181532395*T;
...
result[1] = x12 + x17*(26000.0*n1 + x15*(x10*(x0*(-n2*x1 + x5) - x6 + x9) + x4) + x16);
}
...
static void coder_d2gdn2(double T, double P, double n[2], double result[2]) {
    ...
    double x34 = x10*x33;
    double x35 = x13*(26000.0*n1 + x16*(x34 + x4) + x23);

result[0] = x12 - 2.0*x24 + x30*(16.628925236306479*x21 + x29*(x0*(x26 + x28) + x19 + x25 + x20/n1) + 2.0*x3);
result[1] = x12 - 1.0*x24 + x30*(x22 + x29*(x0*(-x13 + x28) + x18 + x25 - x5) + 1.0*x3 + x34 + 1.0*x4 + 26000.0) - 1.0*x35;
result[2] = x12 + x30*(16.628925236306479*T*x33 + x29*(x0*(n2*x27 + x26) + x17 + x31 + x32/n2) + 2.0*x4) - 2.0*x35;
}
...
static void coder_d3gdn3(double T, double P, double n[2], double result[2]) {
    ...
    double x53 = x10*x52;
    double x54 = x13*(26000.0*n1 + x16*(x4 + x53) + x24);
    double x55 = x12 - 2.0*x17*(x23 + 1.0*x3 + 1.0*x4 + x45*x48 + x53 + 26000.0);
...
result[0] = x12 + 6.0*x25 - 3.0*x36 + x46*(24.943387854459719*x34 + x45*(x0*(x38 + x40) + x31*x32 + x37 + x44 - x21/((n1)*(n1))));
...
result[3] = x12 + x46*(24.943387854459719*T*x58 + x45*(n2*x42 + x0*(-n2*x39 + x38) - x30 + x37 + x50*x57 + x56*x57 - x51/((n2)*(n2)))) + 6.0*x54 - 3.0*x60;
}
...
result = 1.0*x0*(1.0*n1 + 1.0*n2)*(8.3144626181532395*n1*log(n1*x0) + n1*(*endmember[0].dmu0dT)(T, P) + 8.3144626181532395*n2*log(n2*x0) + n2*(*endmember[1].dmu0dT)(T, P));
    return result;
}
...
static void coder_d2gdndt(double T, double P, double n[2], double result[2]) {
    ...
    double x3 = n2*x2;
    double x4 = n1*x2;
    double x5 = 8.3144626181532395*log(x4);
    double x6 = pow(x1, -2);
    ...
    double x12 = 8.3144626181532395*log(x3);
    double x13 = n1*x0 + n1*x5 + n2*x11 + n2*x12;
    double x14 = -1.0*x13*x6*x8 + x13*x9;

result[0] = x10*(x0 - 8.3144626181532395*x3 + x5 + x7*(-n1*x6 + x2)) + x14;
result[1] = x10*(x11 + x12 - 8.3144626181532395*x4 + x7*(-n2*x6 + x2)) + x14;
}
...
static void coder_d3gdn2dt(double T, double P, double n[2], double result[2]) {
    ...
    double x9 = x8*(x1 - x7);
    double x10 = x2 - 8.3144626181532395*x3 + x5 + x9;
    double x11 = x1*x10;
    double x12 = 8.3144626181532395*x1;
    double x13 = 8.3144626181532395*n2;
    double x14 = 8.3144626181532395*n1 + x13;
    double x15 = -2*x6;
    double x16 = pow(x0, -3);
    double x17 = 2*x16;
    double x18 = n1*x17;
    double x19 = x13*x6;
    double x20 = 8.3144626181532395*x7;
    double x21 = x19 - x20;
    ...
    double x32 = x27 + 8.3144626181532395*x28 + x31 - 8.3144626181532395*x4;
    double x33 = x1*x32;
    double x34 = x32*x6;
...
static void coder_d4gdn3dt(double T, double P, double n[2], double result[2]) {
    ...
    double x3 = 8.3144626181532395*n1;
    double x4 = 8.3144626181532395*n2;
    double x5 = x3 + x4;
    double x6 = pow(x0, -2);
    double x7 = -2*x6;
    double x8 = pow(x0, -3);
    ...
    double x11 = x10 + x7;
    double x12 = 1.0/n1;
    double x13 = -8.3144626181532395*n1*x6 + 8.3144626181532395*x1;
    double x14 = x0*x13;
    double x15 = x4*x6;
    double x16 = x3*x6;
    double x17 = x15 - x16;
    ...
    double x22 = -n2*x2 + x14 + x20 + 8.3144626181532395*x21;
    double x23 = x22*x6;
    double x24 = -33.257850472612958*x6;
    double x25 = 6*x8;
    double x26 = pow(x0, -4);
    double x27 = 6*x26;
    double x28 = -n1*x27;
    double x29 = 8.3144626181532395*x0;
    double x30 = x12*x29;
    double x31 = 33.257850472612958*x8;
    double x32 = 16.628925236306479*x8;
    ...
    double x46 = -8.3144626181532395*n2*x6 + 8.3144626181532395*x1;
    double x47 = x0*x46;
    double x48 = -n1*x2 + x42 + 8.3144626181532395*x43 + x47;
    double x49 = x48*x6;
    double x50 = x10 - x6;
    double x51 = 2.0*x8;
    double x52 = x35*x48;
...
result[1] = 1.0*x19 - 4.0*x23 - x36*x40 + x37*(x30*x50 + x34 + x5*(x28 + 4*x8) - 16.628925236306479*x6) + x39*x53 - 2.0*x49 + x51*x52 + x55;
result[2] = -2.0*x23 - x36*x60 + x37*(x33 + x5*(x28 + x9) + 8.3144626181532395*x6) + x39*x51 - 4.0*x49 + x52*x53 + x55 + 1.0*x59;
