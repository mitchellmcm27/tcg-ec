...
result = 1.0*x1*(4700.0*n1*n2 + 1.0*x0*(16.628925236306479*T*(n1*log(n1*x1) + n2*log(n2*x1)) + n1*(*endmember[0].mu0)(T, P) + n2*(*endmember[1].mu0)(T, P)));
    return result;
}
...
result[1] = x12 + x17*(4700.0*n1 + x15*(x10*(x0*(-n2*x1 + x5) - x6 + x9) + x4) + x16);
}
...
static void coder_d2gdn2(double T, double P, double n[2], double result[2]) {
    ...
    double x34 = x10*x33;
    double x35 = x13*(4700.0*n1 + x16*(x34 + x4) + x23);

result[0] = x12 - 2.0*x24 + x30*(33.257850472612958*x21 + x29*(x0*(x26 + x28) + x19 + x25 + x20/n1) + 2.0*x3);
result[1] = x12 - 1.0*x24 + x30*(x22 + x29*(x0*(-x13 + x28) + x18 + x25 - x5) + 1.0*x3 + x34 + 1.0*x4 + 4700.0) - 1.0*x35;
...
static void coder_d3gdn3(double T, double P, double n[2], double result[2]) {
    ...
    double x53 = x10*x52;
    double x54 = x13*(4700.0*n1 + x16*(x4 + x53) + x24);
    double x55 = x12 - 2.0*x17*(x23 + 1.0*x3 + 1.0*x4 + x45*x48 + x53 + 4700.0);
